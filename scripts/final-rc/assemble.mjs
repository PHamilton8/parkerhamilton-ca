...
function parseArgs(argv) {
  const args = { manifest: null, mode: 'DRY-RUN', integrationBranch: null, outputDir: null };
...
function exec(command, args, options = {}) {
  ...
  const result = spawnSync(command, args, { cwd: options.cwd, env: { ...process.env, ...(options.env || {}) }, encoding: 'utf8', input: options.input, maxBuffer: 64 * 1024 * 1024, stdio: options.stdio });
...
function validateManifestShape(manifest, policy, args) {
  ...
  } else if (args.integrationBranch) throw new RcError('--integration-branch is accepted only with --mode EXECUTE.');
}
...
function runGate(worktree, testSummary, logDir, phase, command, args) { const safeName = `${String(testSummary.length + 1).padStart(3, '0')}-${phase}-${[command, ...args].join('-').replace(/[^A-Za-z0-9._-]+/g, '_')}.log`; const logFile = path.join(logDir, safeName); const result = exec(command, args, { cwd: worktree, allowFailure: true, logFile }); testSummary.push({ phase, name: [command, ...args].join(' '), exitCode: result.exitCode, durationMs: result.durationMs, logFile }); if (result.exitCode !== 0) throw new RcError(`${phase}: gate failed: ${[command, ...args].join(' ')}`, result); }
function runWaveTests(worktree, profile, lane, testSummary, logDir) { const scripts = packageScripts(worktree); const phase = `wave-${lane}`; const runScript = (script) => { if (!scripts.has(script)) throw new RcError(`${phase}: required npm script '${script}' is missing.`); runGate(worktree, testSummary, logDir, phase, 'npm', ['run', script]); }; runScript('check'); runScript('test:source'); if (profile === 'modelAware') runScript('test:engines'); if (profile === 'qa' || profile === 'infra') runScript('test'); if (profile === 'infra') runScript('public-safety'); runScript('build'); if (profile === 'infra') { if (scripts.has('infra:validate')) runScript('infra:validate'); if (scripts.has('infra:validate-dist')) runScript('infra:validate-dist'); } }
function runFinalHarness(worktree, testSummary, logDir) {
  ...
  for (const script of ['check','test','public-safety','qa:contrast','build']) { if (!scripts.has(script)) throw new RcError(`${phase}: required npm script '${script}' is missing.`); runGate(worktree, testSummary, logDir, phase, 'npm', ['run', script]); }
  for (const optional of ['infra:validate','infra:validate-dist']) if (scripts.has(optional)) runGate(worktree, testSummary, logDir, phase, 'npm', ['run', optional]);
...
function main() {
  ...
  try {
    ...
    for (const lane of laneInspections) {
      ...
      if (lane.lane === 'releaseTooling') ensureNpmInstalled(worktree, summary.tests, logDir, `post-${lane.lane}`);
    ...
    if (args.mode === 'EXECUTE') { git(repoRoot, ['branch', args.integrationBranch, summary.finalCandidateSha]); const push = git(repoRoot, ['push', '--set-upstream', 'origin', args.integrationBranch], { allowFailure: true }); if (push.exitCode !== 0) { git(repoRoot, ['branch', '-D', args.integrationBranch], { allowFailure: true }); throw new RcError(`Validated RC could not be pushed to ${args.integrationBranch}; local branch was removed and no force push was attempted.`, push); } summary.pushedIntegrationBranch = args.integrationBranch; }
