...
const approvedDownloadHashes = new Map([
  ...
  ['public/downloads/Smith_Manoeuvre_Public_Sanitized_V2.xlsx', '3eb26f8f76c6dbc16ca42ec10debbb7d8b9f5b56fe9169fc55c49618a98c6ef9'],
]);
...
for (const file of files) {
  ...
  if (/\.(?:xlsx?|xlsm|docx?|zip|env|pem|key|sqlite|db)$/i.test(file)) {
    ...
    if (!expectedHash || actualHash !== expectedHash) failures.push([file, 'Private-source or credential file type (no exact approved download match)']);
  }
  if (bytes.includes(0)) continue;
  const text = bytes.toString('utf8');
  for (const [reason, pattern] of textRules) if (pattern.test(text)) failures.push([file, reason]);
  if (/^(?:src|public)\//.test(file)) {
    if (/MSc Candidate|Breach Reporting|askwill\.ca\s*\/\s*staging|LinkedIn will be added/i.test(text)) failures.push([file, 'Unapproved public wording']);
...
if (failures.length) {
  ...
  console.log(`PASS: ${files.length} repository files scanned; approved public workbooks match exact frozen hashes and no flagged credentials, private-source files, local paths, analytics IDs, or unapproved public wording were found.`);
}
