...
# Reviewed current-authority QA and exact publication supplement

The successor keeps the original immutable base, lane order, file ownership and frozen-data policy. Before the final tooling lane lands, it executes the current source-contract assertions from that lane's exact committed source blob and SHA-256 pin outside the candidate tree. The assertion phase is the exact already-applied lane prefix. Historical assertions apply only to not-yet-applied lanes; no old/new alternative is accepted. Every unchanged source companion suite, financial regression suite, build and safety gate remains required.

Complete final-lane runs require explicit `final-rc` browser authority and a nonempty, zero-skip, zero-flaky, zero-failure JSON report with equal assertion coverage in Chromium, Firefox and WebKit. The existing deliberately partial rehearsal returns `PREP_ONLY` browser status and can never authorize publication.

`EXECUTE` additionally requires `--expected-dry-run-summary <path>` identifying the complete green DRY-RUN packet. After independently rerunning all gates, the assembler compares the exact source SHA, tree, lane lineage, changed-file manifest and every dist-manifest entry before creating or pushing the integration branch. Any mismatch stops publication.

The reviewed `final-rc-certified-integration` workflow runs only on `codex/final-rc-certified-integration`, with a committed `scripts/final-rc/manifest.final-approved.json` containing every exact final lane. It runs DRY-RUN first, then reproduces and publishes only `integration/final-rc-v1` through GitHub Actions checkout authorization. It makes no Cloudflare request and performs no production deployment. Keep this operator manifest branch separate from the pinned green release-tooling candidate input to avoid a self-referencing source SHA.
