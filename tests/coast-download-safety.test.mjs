...
const approvedPath = 'public/downloads/Coast_FI_Calculator_Public_Sanitized.xlsx';
const approvedBytes = readFileSync(new URL(`../${approvedPath}`, import.meta.url));
function scan(path, bytes) {
  const root = mkdtempSync(join(tmpdir(), 'coast-download-safety-'));
  try {
...
test('approved Coast workbook passes only at its approved path with exact bytes', () => {
...
test('unapproved workbook names and private source types remain rejected', () => {
  for (const name of ['other.xlsx', 'original.xlsm', 'source.zip', 'private.docx']) {
